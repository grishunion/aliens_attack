...
class AlienInvasion:
    ...
    def _check_keyup_events(self, event):
        """Реагирует на отпусканине клавиш"""
        if event.key == pygame.K_RIGHT:
    ...
    def _update_bullets(self):
        """Обновление позиции снарядов и уничтожает стврые снаряды"""
        #Обновление позиций снарядов. 
        self.bullets.update()

        #Удаленние снарядов, вышедших за край экрана.
        for bullet in self.bullets.copy():
            if bullet.rect.bottom <= 0:
                self.bullets.remove(bullet)  

        # Проверка попаданий пришельцев
        # При обнаружении попадания удалтьб снаряд и пришельца
